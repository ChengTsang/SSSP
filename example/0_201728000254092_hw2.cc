...
public:
	...
	//get val
		...
		for(; !pmsgs->done(); pmsgs->next()){
			Int_Min = min(pmsgs->getValue(), Int_Min);
		}
		if(Int_Min < getValue()){
			val = Int_Min;
			}
	...
	for(; !iter.done(); iter.next()){
		if(val == INT_MAX)
			sendMessageTo(iter.target(), val);
		else
			sendMessageTo(iter.target(), val+iter.getValue());
		}
	}   
};
